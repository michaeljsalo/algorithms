...
function permutateLoop1 (chars = 'aaa') {
    ...
    for (let charA of threeChars) {
        for (let charB of threeChars) {
            for (let charC of threeChars) {
...
function permutateLoop2 (chars = 'aaa') {
    const permutations = [];
    let remain3 = chars.slice(0, 3);

    for (let a = 0; a < remain3.length; a += 1) {
        let remain2 = remain3.slice(0, a) + remain3.slice(a + 1);

        for (let b = 0; b < remain2.length; b += 1) {
            let remain1 = remain2.slice(0, b) + remain2.slice(b + 1);
